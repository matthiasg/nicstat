...
typedef struct nicdata {
  ...
  zonename zone;    /* name of zone of this interface */
} nicdata;
 ...
 * Global variables
 */
static char *g_network[] = { "link", "be", "bge", "ce", "ci", "dmfe", "e1000g", "el",
	"eri", "elxl","rge", "fa", "ge", "hme", "ipge", "ipdptp", "iprb", "lane",
...
{
	if (g_style)
		(void) printf("%8s %11s %36s %14s %14s\n",
		    "Time", "Int", "Zone", "rKb/s", "wKb/s");
	else
		(void) printf(
		    "%8s %11s %36s %7s %7s %7s %7s %7s %7s %7s %7s\n",
		    "Time", "Int", "Zone", "rKb/s", "wKb/s", "rPk/s",
		    "wPk/s", "rAvs", "Zone", "%Util", "Sat");
}
  ...
  time_t boot_time;	/* boot time */
	...
	for (;;)
	{
		...
		for (num = 0; num <= g_interfacemax; num++) {
			...
			/* Print output line */
			(void) printf("%s %11s %36s ", timestr, g_idnew[num].name, g_idnew[num].zone);
			if (g_style)
